...
def test_delete_live_tournament_requires_force(admin_env):
    """A LIVE tournament's source is deletable ONLY with explicit
    {"force": true} — without it the delete is refused so scripts can't
    wipe a live source by accident."""
    token = admin_env["admin_token"]
    pt = admin_env["publish_token"]
    base = admin_env["base"]
    ...
    # Without force → refused, source survives
    s, d = req("DELETE", f"{base}/api/tournament/{org}/{slug}", token=token,
               publish_token=pt)
    assert s == 400, d
    assert "force" in d["error"]
    assert os.path.isdir(tdir), "live source must survive a refused delete"

    # With force → deleted, and the response explains the app repo stays
    s, d = req("DELETE", f"{base}/api/tournament/{org}/{slug}", token=token,
               publish_token=pt, body={"force": True})
    assert s == 200, d
    assert d["deleted"] == f"{org}/{slug}"
    assert "app repo untouched" in d["note"]
    assert not os.path.isdir(tdir), "live source must be gone after force delete"

    # Audit records the live deletion
    with open(os.path.join(admin_env["content"], "out", "audit.log")) as f:
        log = f.read()
    assert '"action": "tournament.delete"' in log
    assert "live=True" in log
